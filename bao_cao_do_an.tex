...
\renewcommand{\arraystretch}{1.25} % tăng độ cao hàng cho thoáng

\begin{tabular}{|>{\raggedright\arraybackslash\ttfamily}p{3.8cm}|
                >{\raggedright\arraybackslash}p{3cm}|
...
Xử lý missing values với các chiến lược:
\textbf{mean} (trung bình),
\textbf{median} (trung vị),
\textbf{mode} (giá trị xuất hiện nhiều nhất),
\textbf{constant} (giá trị cố định).
Khi \texttt{fit=True}, tính và lưu giá trị thống kê. \\
\hline

remove\_outliers() & method, threshold &
Loại bỏ outliers bằng:
\textbf{IQR} (1.5×IQR),
\textbf{z-score} (ngưỡng = 3),
\textbf{Isolation Forest}.
Áp dụng trên tập train. \\
\hline
...
Pipeline hoàn chỉnh:
fill missing → encode categorical → scale features.
Tách biến mục tiêu ra khỏi features. \\
\hline
...
save\_state() & path &
Lưu toàn bộ trạng thái (impute values, fitted encoders, scalers) bằng joblib. \\
\hline

load\_state() & path &
Tải trạng thái đã lưu để transform dữ liệu mới mà không cần train lại. \\
\hline
...
\hline
Polynomial Regression & 14.64 & 13.58 & 10.71 & 0.837 \\
\hline
Random Forest & 4.89 & 7.78 & 4.94 & 0.946 \\
\hline
Extra Trees & 1.19 & 7.71 & 4.60 & 0.947 \\
\hline
\textbf{XGBoost} & \textbf{6.23} & \textbf{6.77} & \textbf{4.36} & \textbf{0.959} \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Test RMSE = 6.77}: Sai số điển hình khoảng 6.77 đơn vị tiền tệ
    \item \textbf{Test MAE = 4.36}: Sai số trung bình khoảng 4.36 đơn vị tiền tệ
    \item \textbf{Test R² = 0.959}: Giải thích được 95.9\% phương sai của giá cước
\end{itemize}
...
\begin{itemize}
    \item \textbf{XGBoost}: Train RMSE = 6.23, Test RMSE = 6.77 → Gap = 0.54 (tổng quát hóa rất tốt)
    \item \textbf{Extra Trees}: Train RMSE = 1.19, Test RMSE = 7.71 → Gap = 6.52 (overfitting đáng chú ý)
    \item \textbf{Random Forest}: Train RMSE = 4.89, Test RMSE = 7.78 → Gap = 2.89 (overfitting nhẹ)
    \item \textbf{Polynomial}: Train RMSE = 14.64, Test RMSE = 13.58 → Gap = -1.06 (hơi underfit)
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Đạt R² = 0.837 – đủ tốt cho baseline tuyến tính
        \item Gap âm nhỏ (train > test) cho thấy mô hình hơi underfit → phù hợp minh hoạ, không nên deploy
        \item Ưu điểm chính là dễ giải thích và training cực kỳ nhanh
    \end{itemize}
    
    \item \textbf{XGBoost:}
    \begin{itemize}
        \item Test RMSE thấp nhất (6.77) và R² cao nhất (0.959)
        \item Generalization rất tốt (gap chỉ 0.54) nhờ chiến lược xử lý dữ liệu clip/drop
...
Polynomial &
\begin{tabular}[t]{@{}l@{}}
\hspace{0.4cm}degree = 3 \\
\hspace{0.4cm}alpha = 1.0 \\
\hspace{0.4cm}feature\_subset = [Trip\_Distance\_km, \\
\hspace{1.4cm}Per\_Km\_Rate, \\
\hspace{1.4cm}Trip\_Duration\_Minutes]
\end{tabular}
\\
\hline
...
\renewcommand{\arraystretch}{1.35} % tăng chiều cao hàng
\setlength{\tabcolsep}{10pt}       % tăng đệm ngang cho mỗi ô
...
Vẽ histogram với KDE (Kernel Density Estimation) cho tất cả biến số.
Hiển thị mean, median, std giúp nhận diện phân phối và skewness. \\
\hline

\texttt{plot\_categorical\_grid()} &
Vẽ bar chart cho các biến phân loại. Hiển thị tần suất của từng category,
hỗ trợ phát hiện class imbalance. \\
\hline

\texttt{plot\_correlation\_heatmap()} &
Vẽ ma trận tương quan (correlation matrix) dạng heatmap.
Giúp nhận diện multicollinearity và các features quan trọng. \\
\hline

\texttt{plot\_target\_analysis()} &
Phân tích biến mục tiêu: distribution, Q–Q plot, box plot.
Kiểm tra normality và outliers của target. \\
\hline

\texttt{plot\_outliers\_boxplot()} &
Vẽ boxplot cho tất cả biến số. Hiển thị Q1, Q3, IQR và số lượng outliers. \\
\hline

\texttt{plot\_data\_overview()} &
Tổng quan dữ liệu: shape, dtypes, tỷ lệ missing values theo từng cột. \\
\hline

\texttt{plot\_metrics\_summary()} &
So sánh các metrics (RMSE, MAE, R²) của tất cả mô hình bằng grouped bar chart. \\
\hline

\texttt{plot\_predictions\_combined()} &
Scatter plot Actual vs Predicted cho tất cả mô hình. Bao gồm đường y=x
để đánh giá độ chính xác. \\
\hline

\texttt{plot\_feature\_importance()} &
So sánh feature importance của các tree-based models.
Giúp nhận diện các features ảnh hưởng mạnh nhất. \\
\hline
...
\begin{enumerate}
    ...
    \item \textbf{So sánh mô hình}: Huấn luyện và đánh giá 4 mô hình, với \textbf{XGBoost} đạt hiệu suất tốt nhất (R² = 0.959, RMSE = 6.77).
